#include <iostream>
#include <vector>
#include <stack>

struct Node {
    enum Color {
        white = 0, gray, black
    };
    std::vector<size_t> connected;
    Color color = white;

    size_t prev;
};

void visit(std::vector<Node> &graph, size_t index) {
    graph[index].color = Node::gray;

    for (auto &node : graph[index].connected) {
        if (graph[node].color == Node::white) {
            graph[node].prev = index;
            visit(graph, node);
        } else if (graph[node].color == Node::gray) {
            std::cout << "YES\n";
            std::stack<size_t> path;

            for (size_t v = index; v != node; v = graph[v].prev) {
                path.push(v);
            }
            path.push(node);

            while (!path.empty()) {
                std::cout << path.top() + 1 << ' ';
                path.pop();
            }

            exit(0);
        }
    }

    graph[index].color = Node::black;
}

void dfs(std::vector<Node> &graph) {
    for (size_t i = 0; i < graph.size(); ++i) {
        if (graph[i].color == Node::white) {
            visit(graph, i);
        }
    }
}

int main() {
    std::freopen("cycle.in", "r", stdin);
    std::freopen("cycle.out", "w", stdout);

    size_t n, m;
    std::cin >> n >> m;

    std::vector<Node> graph(n);

    for (size_t i = 0; i < m; ++i) {
        size_t x, y;
        std::cin >> x >> y;
        --x; --y;

        graph[x].connected.push_back(y);
    }

    dfs(graph);

    std::cout << "NO";

    return 0;
}
